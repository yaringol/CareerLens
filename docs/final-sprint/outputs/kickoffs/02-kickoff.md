...
## 11. ממצאים מחוץ לסקופ

> **עדכון 22/08: שני באגי הזיהוי תוקנו בהוראת המשתמשת** (`f1e0859`, `e877c72`) - ראה
> סעיף 12. שני ה-controllers המתים עדיין ממתינים להכרעה.
...
## 12. תיקון שני באגי הזיהוי (22/08, בהוראת המשתמשת)

### באג 1 - `f1e0859` (`ds/model/server.py`)

**הפתרון:** `normalize_title_semantic_topk` מפצל את הכותרת למקטעים
(`, / | & and –  - `) ומדרג כל טייטל קנוני לפי **המקסימום** על פני הביטוי המלא והמקטעים.
כותרת בלי מפרידים מייצרת וריאנט אחד ומתנהגת בדיוק כמו קודם - ולכן הסיכון מרוכז רק
בכותרות המורכבות.

**חלופות שנדחו:** לקחת רק את המקטע הראשון (שובר `"Tech Lead, Senior Backend Developer"`);
לאמן מחדש את ה-normalizer (יקר, ומשנה את כל 59 הסנטרואידים).

**מדידה על 32 הקו"ח האותנטיים דרך הצינור האמיתי** (`01-title-benchmark` +
`02-title-metrics`, תגיות `m02-before`/`m02-after`):

| מדד | לפני | אחרי |
|---|---|---|
| **Top-1** | 26/29 (89.7%) | **27/29 (93.1%)** |
| Top-3 | 27/29 (93.1%) | 27/29 (93.1%) |
| clear-cut | 8/9 | **9/9** |
| שאר התרחישים | - | ללא שינוי |

`03-compare-runs`: **28/32 זהים, 4 שינויים - כולם לטובה**, אף קו"ח לא עבר לטייטל שגוי:

- `Daniel-Peretz` **Frontend 73.73 → Backend 84.9** (הבאג עצמו)
- `Nadav-Stern` DevOps 83.4 → **96.96** (אותו טייטל, ביטחון גבוה יותר)
- `Alex-Vaisman` DevOps 82.14 → **90.24**
- `Gal-Nissim` Data Scientist 86.92 → **89.75** (הכותרת מכילה `/`)

### באג 2 - `e877c72` (`CvUploadSection.tsx`)

השומר הוחלף מ"רק `uncertain`/`not-found`" ל"כל מצב שנפתר" (חוסם רק `idle`/`detecting`),
וה-`detectedTitle` נשמר מהכותרת שנקראה מהקו"ח גם במסלול ה-`ready`.

**אימות ב-Playwright:** קו"ח Peretz → זוהה `Backend Developer 84.9%` (באג 1 מתוקן חי) →
פתיחת הבורר הידני → `Data Engineer 96.43%` → **התצוגה התעדכנה ל-Data Engineer**. לפני
התיקון הלחיצה לא שינתה כלום.

### ⚠️ תוצאת לוואי שדורשת טיפול

`official-metrics.md` והספר מצטטים **Top-1 89.7%**. אחרי התיקון המספר האמיתי הוא
**93.1%**. צריך להחליט: לרענן את המדד בשני המסמכים (עדיף - המספר השתפר), או לתעד
במפורש שהמדידה קדמה לתיקון. **חוסם את M12.**
